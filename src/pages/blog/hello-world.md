...
"Resolving Kubernetes Pod OOMKilled Errors for Java Spring Boot Applications: Conquering Out of Memory Issues with Configurations"
We encountered an issue with our Java Spring boot application where the kuberenetes pods were getting killed or restarted with an OOMKilled error and an Exit Code of 137.
...
After much contemplation and exploration, we made a crucial discovery. It dawned upon us that our application was not the sole occupant within the pod. We realized that we were utilizing the Amazon Corretto provided JAVA Docker image to run our application. This Java image required a substantial amount of memory, exceeding 300MB, to perform certain background tasks related to Java itself.
